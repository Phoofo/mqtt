...
//            ctx.writeAndFlush(response);
    ...
    private ByteBuf processRequest(String request, ChannelHandlerContext ctx) {
        ...
        if (request.length() == 16) {
            ...
            log.error("报文消息格式错误:{}", request);
            throw new IllegalArgumentException("报文消息格式错误");
        }
        //根据IP、端口和地址设置硬件的状态
        ...
        // 使用Bean实例进行操作
        try {
            if (StringUtils.isEmpty(map.get("2")) || StringUtils.isEmpty(map.get("6"))) {
                log.error("报文消息截取错误:{}", map);
                throw new IllegalArgumentException("报文消息截取错误");
            }
